...
v0.3.2 特意保留了 `context7` MCP。受影响最重的恰恰是主力 provider codex。

**因此采纳的决定是（见 §四）：交互式仍为默认，结构化作为辅助路径先在 gemini /
claude 试点，未来按明确的触发条件再评估是否提权。** 本文的技术结论（结构化可以替代
抓屏）成立，但落地节奏取决于主力路径能不能受益——现在还不能。
