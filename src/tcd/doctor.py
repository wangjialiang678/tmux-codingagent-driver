...
def _check_providers(report: DoctorReport) -> dict[str, Any]:
    ...
    for name in list_providers():
        ...
        report.add(
            ...
            working_markers=list(getattr(prov, "working_markers", ())),
...
def _get_version(command: str, flags: tuple[str, ...] = ("--version", "-V")) -> tuple[str | None, str | None]:
    """Run a bounded version command and return its first non-empty line.

    Tries each flag in turn: `--version` is near-universal but tmux only
    accepts `-V`, and reporting "version unknown" for a tool that is installed
    and working is exactly the kind of noise that gets a health check ignored.
    """
    last_error = "no version flag succeeded"
    for flag in flags:
        try:
        ...
        if result.returncode != 0:
            last_error = output or f"exited with status {result.returncode}"
            continue
        if not output:
